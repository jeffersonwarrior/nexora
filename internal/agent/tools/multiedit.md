...
2. edits: Array of edit objects (MUST be an array, not a string!):
   ...
   ```
   "edits": [
     {"old_string": "text1", "new_string": "replace1"},
     {"old_string": "text2", "new_string": "replace2"}
   ]
   ```
   
   ❌ INCORRECT (will fail):
   ```
   "edits": "[{\"old_string\": \"text1\", \"new_string\": \"replace1\"}]"
   ```
</parameters>
